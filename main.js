...
io.on('connection', function(client){
    online_users++
    var log__user_name;
    var log__client_id = client.id.substr(-4);
    io.sockets.emit('users', online_users);
    setTimeout(function(){
        if (log__user_name == undefined){
            client_log(log__client_id + " joined")
        }
    }, 200)
    ...
    client.on('login', function(user, lang){
        log__user_name = user;
        io.sockets.emit('users', online_users);
        client_log(log__client_id + " joined as " + log__user_name)
        
        client.on('get_quizz', function(screen_name){

            client_log(log__client_id + " vient de commencer un quizz :" + screen_name)
    ...
    client.on('disconnect', function(){
        ...
        if (log__user_name == undefined){
            client_log(log__client_id + " left")
        }else{
            client_log(log__user_name + " left")
        }

    });

    function client_log(log){
        io.sockets.emit('log', log__client_id + " joined")
    }
});
